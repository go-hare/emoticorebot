...
async def _exercise_output_runtime_builds_stream_events() -> None:
    ...
    await bus.publish(
        build_envelope(
            ...
            payload=MainBrainStreamDeltaPayload(
                stream_id="stream_1",
                delta_text="你好。",
                stream_state="open",
                stream_index=1,
    ...
    await bus.publish(
        build_envelope(
            ...
            correlation_id="turn_stream",
            payload=MainBrainReplyReadyPayload(
                request_id="main_brain_reply_stream",
                reply_text="你好。我在这。",
    ...
    await bus.publish(
        build_envelope(
            ...
            payload=MainBrainFollowupReadyPayload(
                job_id="job_2",
                source_event=EventType.EXECUTION_EVENT_RESULT_READY,
...
async def _exercise_output_runtime_emits_safe_fallback_for_blocked_reply() -> None:
    ...
    await bus.publish(
        build_envelope(
            ...
            payload=MainBrainReplyReadyPayload(
                request_id="main_brain_reply_unsafe",
...
async def _exercise_output_runtime_skips_suppressed_left_reply() -> None:
    ...
    await bus.publish(
        build_envelope(
            ...
            payload=MainBrainReplyReadyPayload(
                request_id="main_brain_reply_sync",
    ...
    await bus.publish(
        build_envelope(
            ...
            payload=MainBrainFollowupReadyPayload(
                ...
                source_event=EventType.EXECUTION_EVENT_PROGRESS,
                source_decision="accept",
