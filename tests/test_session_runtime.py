...
async def _exercise_session_runtime_tracks_right_brain_flow() -> None:
    ...
    bus.subscribe(consumer="brain", event_type=EventType.LEFT_COMMAND_REPLY_REQUESTED, handler=_capture)
    ...
    assert followups[2].payload.followup_context.preferred_delivery_mode == "push"
...
async def _exercise_session_runtime_keeps_answer_only_delivery_mode() -> None:
    ...
    record = store.add(
        RightBrainRecord(
            ...
            delivery_target=DeliveryTargetPayload(delivery_mode="inline"),
        )
    )
    session = SessionRuntime(bus=bus, task_store=store)
    session.register()
    ...
    assert followup.preferred_delivery_mode == "inline"
