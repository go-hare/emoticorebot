...
- 本轮启用的专家列表，以及 EQ 仲裁结果（`accepted_experts` / `rejected_experts` / `arbitration_summary`）
- 指向完整 IQ 审计包的 `iq_audit_id` / `iq_audit_path`

完整的 IQ 内部 payload（`expert_packets`、`tool_calls`、分歧摘要、overlay 锚点等）会单独保存到 `session_audits/`，这样热路径上的 `sessions/*.jsonl` 会更轻。
