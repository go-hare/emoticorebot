...
def build_reflection_input(payload: Mapping[str, Any] | None) -> ReflectionInput:
    ...
    reflection_input: ReflectionInput = {
        ...
        "brain": _normalize_brain_packet(data.get("brain")),
        "task": task,
...
def _normalize_brain_packet(value: Any) -> BrainControlPacket:
    payload = _normalize_dict(value)
    if not payload:
        return {}

    normalized: BrainControlPacket = {}
    ...
    if final_decision in {"answer", "ask_user", "continue"}:
...
def _normalize_execution_info(value: Any) -> ExecutionInfo | None:
    ...
    missing = _normalize_str_list(payload.get("missing"))
    summary = _as_text(payload.get("summary"))
    failure_reason = _as_text(payload.get("failure_reason"))
    recommended_action = _as_text(payload.get("recommended_action"))
    ...
    if not invoked and not any((summary, missing, failure_reason, recommended_action, attempt_count)):
        return {
            "invoked": False,
            "status": "none",
            "summary": "",
...
def _build_execution_info(
    ...
    recommended_action = _as_text(task.get("recommended_action"))
    confidence = _safe_float(task.get("confidence"), default=0.0)
    attempt_count = _safe_int(task.get("attempt_count"), default=0)
    ...
    if attempt_count <= 0 and invoked:
        attempt_count = 1
...
def _normalize_task_state(value: Any) -> dict[str, Any]:
    ...
    input_request = _normalize_dict(payload.get("input_request"))
    if input_request:
        field = _as_text(input_request.get("field"))
        question = _as_text(input_request.get("question"))
        if field or question:
            normalized["input_request"] = {"field": field, "question": question}

    pending_review = [dict(item) for item in list(payload.get("pending_review", []) or []) if isinstance(item, dict)]
    if pending_review:
        normalized["pending_review"] = pending_review
