...
from emoticorebot.main_brain.packet import DecisionPacket as MainBrainDecisionPacket, TaskAction as MainBrainTaskAction
...
class ReflectionInput(TypedDict, total=False):
    ...
    main_brain: MainBrainDecisionPacket  # 主脑决策包
...
class ReflectionStateUpdate(TypedDict, total=False):
    """主脑对本轮状态记录结果的判断。"""

    should_apply: bool  # 是否建议同步到实时状态
    confidence: float  # 应用该更新的置信度
    reason: str  # 更新原因
    pad_state: dict[str, float]  # PAD 状态值
    drives_state: dict[str, float]  # drives 状态值


class MemoryCandidate(TypedDict, total=False):
    """可写入长期记忆的候选项。"""

    memory_type: MemoryType  # 正式长期记忆类型
    summary: str  # 一句话摘要
    detail: str  # 更完整提炼内容
