"""Right-brain runtime that owns execution runs and lifecycle control."""
...
from emoticorebot.right_brain.hooks import AuditInterrupt, AuditSignal
from emoticorebot.utils.right_brain_projection import project_task_from_runtime_snapshot


class RightBrainRuntime:
    """Constant resident runtime that starts and controls right-brain runs."""
    ...
    def __init__(
        ...
        *,
        ...
        executor: RightBrainExecutor | None = None,
    ) -> None:
        self._bus = bus
        ...
        if self._executor is None and executor_llm is not None and context_builder is not None:
            self._executor = RightBrainExecutor(executor_llm=executor_llm, tool_registry=tool_registry, context_builder=context_builder)
    ...
    async def _dispatch_right_job(self, event: BusEnvelope[RightBrainJobRequestPayload]) -> None:
        ...
        if self._executor is None:
            await self._publish_rejected(
                ...
                reason="RightBrainRuntime 当前没有可用的右脑执行器。",
            )
            record.mark_done(
                result="failed",
                summary="右脑执行器不可用",
                error="RightBrainRuntime 当前没有可用的右脑执行器。",
                decision="reject",
            )
            return
    ...
    def _resolve_delivery_target(
        ...
        *,
        ...
        if delivery_mode not in {"inline", "push", "stream"}:
            raise RuntimeError(f"invalid right brain delivery_mode: {delivery_mode!r}")
    ...
    async def _run_record(self, record: RightBrainRecord, event: BusEnvelope[RightBrainJobRequestPayload]) -> None:
        ...
        async def _on_execution_update(message: str, payload: dict[str, Any]) -> None:
        ...
        self._executor.run_hooks.bind_audit_handler(_on_audit)
        try:
            result = await self._executor.execute(
                self._build_executor_task_spec(record),
                task_id=record.task_id,
                progress_reporter=_on_execution_update,
                trace_reporter=_on_execution_update,
            )
    ...
    async def _publish_execution_progress(
        ...
        *,
        ...
        if record.state is RightBrainState.DONE or not record.accepted:
            return
        ...
        record.touch()
        record.append_trace(
            kind=self._progress_trace_kind(metadata),
            message=summary,
            data={
                "stage": "execute",
    ...
    async def _handle_execution_result(
        ...
        *,
        ...
        if control_state not in {"completed", "failed"}:
            raise RuntimeError(
                "unsupported right-brain control_state: "
                f"{control_state!r}; use audit_tool(decision=\"reject\") for missing-input cases"
            )
        if not record.accepted:
            raise RuntimeError(
                "right-brain execution must call audit_tool(decision=\"accept\") before returning a result"
            )
                    ...
                    metadata={
                        "right_brain_summary": {
                            ...
                            "source_event_type": (
                                ...
                                if record.terminal_decision != "reject"
    ...
    async def _complete_rejected(
        ...
        *,
        ...
        final_reason = str(reason or summary or "").strip() or "右脑拒绝执行当前请求。"
